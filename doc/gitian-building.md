...
*Setup instructions for a gitian build of PCIB using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the PCIB
...
Clone the git repositories for pcib and gitian and then checkout the pcib version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the pcib repository
...
To build PCIB (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the pcib repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pcib/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/pcib-crypto/pcib
...
For example:
```bash
URL=https://github.com/crowning-/pcib.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit pcib=${COMMIT} --url pcib=${URL} ../pcib/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit pcib=${COMMIT} --url pcib=${URL} ../pcib/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit pcib=${COMMIT} --url pcib=${URL} ../pcib/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pcib-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pcib-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/pcib-build.assert
```
...
[pcib/gitian.sigs](https://github.com/pcib-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
