...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("Pivx Core pcib-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  pcib-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded pcib transaction") + "\n" +
              "  pcib-tx [options] -create [commands]   " + _("Create hex-encoded pcib transaction") + "\n" +
              "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded pcib transaction
