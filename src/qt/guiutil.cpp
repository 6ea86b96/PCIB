...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a PCIB address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no PCIB: URI
...
{
    // Convert pcib:// to pcib:
    //
    //    Cannot handle this later, because pcib:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open pcib.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PCIB.lnk";
}
...
{
    return GetAutostartDir() / "pcib.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a pcib.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=PCIB\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the pcib app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add pcib app to startup item list
