...
Value mnbudget(const Array& params, bool fHelp)
{
    ...
    if(strCommand == "prepare")
    {
        ...
        if (params.size() != 7)
            throw runtime_error("Correct usage is 'mnbudget prepare proposal-name url payment_count block_start pcib_address monthly_payment_pcib'");
    ...
    if(strCommand == "submit")
    {
        ...
        if (params.size() != 8)
            throw runtime_error("Correct usage is 'mnbudget submit proposal-name url payment_count block_start pcib_address monthly_payment_pcib fee_tx'");
