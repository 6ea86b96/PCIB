...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("PCIBMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("PCIBMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("PCIBMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pcib-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) 
    {
        ...
        LogPrintf("Running PCIBMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
