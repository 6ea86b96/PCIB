...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"pcibprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"pcibprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid PCIB address or script");
    }
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"pcibaddress\"\n"
            "\nReveals the private key corresponding to 'pcibaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"pcibaddress\"   (string, required) The pcib address for the private key\n"
            "\nResult:\n"
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by PCIB %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
Value bip38encrypt(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"pcibaddress\"\n"
            "\nEncrypts a private key corresponding to 'pcibaddress'.\n"
            "\nArguments:\n"
            "1. \"pcibaddress\"   (string, required) The pcib address for the private key (you must hold the key already)\n"
